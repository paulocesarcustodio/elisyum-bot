...
async function inspect() {
    ...
    const moduleBasePath = path.dirname(fileURLToPath(import.meta.url));
    const projectRoot = path.resolve(moduleBasePath, '..', '..');
    const binaryName = process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp';
    const ytDlpPath = path.join(projectRoot, 'bin', binaryName);
    ...
    console.log('\n8. Trying to instantiate using local binary:', ytDlpPath);
